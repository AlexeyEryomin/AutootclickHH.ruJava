...
public class AutoOtclickController {
    ...
    protected void infinityOtcklick(ActionEvent event) {
        Thread thread = new Thread(new InfinitySuperOtcklick());
        thread.start();
        sendOtclickResult.setText("Отклики отправляются\nпока программа открыта");
    }

    public class InfinitySuperOtcklick implements Runnable {
    ...
    /*public static void infinitySend() {
        int count = 1;
        int pauseTimer = parseInt(pauseTime.getText());
        HTTPRequestHHVacancies getjob = new HTTPRequestHHVacancies();
        String vacancy_name_1 = getVacancyName.getText();
